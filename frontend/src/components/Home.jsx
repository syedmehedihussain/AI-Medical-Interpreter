// Landing screen. Editorial hero, one job: get the clinician into the tool.
// Trust cues are only ones that are true (no HIPAA claim: there is no such cert).
...
function TrustChip({ children, accent }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-pine-200 bg-white px-3.5 py-1.5 text-sm font-medium text-pine-800">
      <span className={`h-1.5 w-1.5 rounded-full ${accent ? 'bg-clay-500' : 'bg-pine-500'}`} aria-hidden="true" />
      {children}
    </span>
  )
}
...
export default function Home({ onStart }) {
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div
        aria-hidden="true"
        className="blob animate-float pointer-events-none absolute -right-24 -top-24 h-72 w-72 bg-pine-300/50 sm:h-[26rem] sm:w-[26rem] lg:-right-16 lg:h-[34rem] lg:w-[34rem]"
      />
      <div
        aria-hidden="true"
        className="blob pointer-events-none absolute -bottom-40 -left-32 h-80 w-80 bg-pine-100"
      />
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-6 sm:px-10">
        <header className="flex items-center justify-between py-6">
          ...
          <span className="hidden rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-xs font-semibold tracking-wide text-slate-600 sm:inline">
            EN <span className="text-pine-600">&harr;</span> <span className="font-bn">বাংলা</span>
          </span>
        </header>

        <main className="flex flex-1 flex-col justify-center py-10 sm:py-16">
          <div className="max-w-3xl animate-rise-in">
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-pine-700">
            ...
            <h1 className="text-display text-5xl font-extrabold text-slate-900 sm:text-6xl lg:text-7xl">
              ...
              <br />
              lost in <span className="text-pine-600">translation.</span>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              <button
                type="button"
                onClick={onStart}
                className="group inline-flex items-center gap-3 rounded-full bg-pine-700 px-8 py-4 text-lg font-semibold text-white shadow-lg shadow-pine-700/25 transition hover:bg-pine-800 active:scale-[0.98]"
              >
        ...
        <section className="grid gap-10 border-t border-slate-200/80 py-12 sm:grid-cols-3 sm:gap-8">
          {FEATURES.map((f) => (
            <div key={f.n}>
              <span className="font-mono text-sm font-semibold text-pine-500">{f.n}</span>
